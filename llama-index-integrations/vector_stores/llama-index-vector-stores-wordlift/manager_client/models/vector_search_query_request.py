...
class VectorSearchQueryRequest(BaseModel):
    ...
    filters: Optional[List[Filter]] = Field(
        default=None, description="A list of prefilters."
    )
    query_embedding: Optional[List[Union[StrictFloat, StrictInt]]] = Field(
        default=None,
        description="The list of embeddings, not required if `query_string` is provided.",
    )
    query_string: Optional[StrictStr] = Field(
        default=None,
        description="The query string, not required if the `query_embeddings` are provided. Please note that the `query_string` is ignored if the `query_embeddings` are provided.",
    )
    similarity_top_k: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(
        default=2, description="The similarity top K."
    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "filters": (
                    [Filter.from_dict(_item) for _item in obj["filters"]]
                ...
                "similarity_top_k": (
                    obj.get("similarity_top_k")
                    if obj.get("similarity_top_k") is not None
                    else 2
                ),
            }
        )
